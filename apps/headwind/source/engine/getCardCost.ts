...
export function getCardCost(game: Game, ship: Ship, card: Card) {
  // console.log('getCardCost()...');
  if (card.config.type === 'move') {
    let windBonus = getWindBonus(game, ship);

    return Math.max(card.config.energy - ship.moveCardCostBonus - windBonus, 0);
  } else if (card.config.type === 'turn') {
    return Math.max(card.config.energy - ship.turnCardCostBonus, 0);
  }

  return card.config.energy;
}
