...
function Coordinates({q, r, s}: {q: number | string; r: number | string; s: number | string}) {
  return (
    <>
      <Text transform={`translate(${0}, ${-3.5})`} style={{...fontStyle, fill: COLORS.red[8]}}>
        {q}
      </Text>
      <Text transform={`translate(${-3}, ${2})`} style={{...fontStyle, fill: COLORS.green[8]}}>
        {s}
      </Text>
      <Text transform={`translate(${3}, ${2})`} style={{...fontStyle, fill: COLORS.blue[8]}}>
        {r}
      </Text>
    </>
  );
      ...
      <HexGrid width={1200} height={800} viewBox="-50 -50 100 100">
        <Layout size={{x: 7, y: 7}} flat={true} spacing={1.1} origin={{x: 0, y: 0}}>
          <>
            ...
            <Hexagon q={0} r={0} s={0} cellStyle={{fill: COLORS.dark[3], stroke: COLORS.dark[4]}}>
              <Coordinates q={'Q'} r={'R'} s={'S'} />
