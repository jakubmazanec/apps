...
function reset() {
  ...
  const hexas: Cell[] = initialHexagons.map(({q, r, s}) => ({
    ...{q, r, s},
    state: (Math.random() < 0.4 && HexUtils.distance({q, r, s}, {q: 0, r: 0, s: 0}) <= 3 ?
      'Living'
    : 'Dead') as State,
  }));
  hexas.forEach((hex) => {
...
function getNeighbors(hex: Cell, dict: CellDict) {
  ...
  return neighbors.map(({q, r, s}) => dict[`${q}-${r}-${s}`]).filter((v) => Boolean(v));
}
  ...
  return (
    ...
    >
      <div>
        <p>
          <b>Living cells: </b>
          Each cell with one or no neighbors dies from isolation. Each cell with three or more
          neighbors dies from overpopulation. Only the cells with two neighbors survive.
        </p>
        <p>
      ...
      <div>
        ...
        <button onClick={() => setSpeed(Math.max(speed - 1, 1))}>+</button> speed: {speed}
      </div>
                ...
                css={css`
                  g {
                    polygon {
                      ...
                      stroke-opacity: ${0.6 / HexUtils.distance(hex, {q: 0, r: 0, s: 0})};
                      stroke-width: 0.15;
