...
describe('static fromIterable()', () => {
  ...
  test('throws when an empty iterable is passed', () => {
    expect(() => Grid.fromIterable([])).toThrowError(
      TypeError(`Can't create grid from empty iterable: []`),
    );
  });
});
...
describe('static fromJSON()', () => {
  ...
  test('accepts an optional hex factory to create custom hexes with', () => {
    ...
    const hexFactory = vi.fn(({q, r, custom}: DeserializedCustomHex) =>
      CustomHex.create([q, r], custom),
    );
    ...
    expect(result).toStrictEqual(
      new Grid(CustomHex, [
        CustomHex.create(coordinates[0], 'a'),
        CustomHex.create(coordinates[1], 'b'),
      ]),
    );
...
describe('pixelWidth property', () => {
  ...
  test('returns the width in pixels', () => {
    ...
    expect(new Grid(PointyHex, rectangle({width: 5, height: 1})).pixelWidth).toBe(
      5 * pointyHexWidth,
    );
    // flat hexes partially overlap in horizontal plane
    expect(new Grid(FlatHex, rectangle({width: 5, height: 1})).pixelWidth).toBe(
      5 * flatHexWidth - flatHexWidth,
    );
  });
});
