 ...
 * @category Traverser
 */
export function line<T extends Hex>(
  options: LineAsVectorOptions | LineBetweenOptions,
): Traverser<T> {
  return isLineVectorOptions(options) ?
      lineFromVectorOptions(options)
    : lineFromBetweenOptions(options);
}
...
function lineFromVectorOptions<T extends Hex>({
  start,
  direction,
  length,
}: LineAsVectorOptions): Traverser<T> {
