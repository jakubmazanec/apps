...
import {distance, neighborOf} from './functions/index.js';
import {concat} from './traversers/index.js';
import {
  type Direction,
  type GridAsJSON,
  type HexIterable,
...
export class Grid<T extends Hex> implements HexIterable<T>, HexTraversable<T> {
  ...
  static fromJSON<T extends AxialCoordinates, R extends Hex>(
    ...
    if (hexFactory) {
      ...
      const HexClass = (
        hexes.length > 0 ?
          hexes[0].constructor
        : hexFactory({q: 0, r: 0} as T, 0, [{q: 0, r: 0}] as T[]).constructor) as HexConstructor<R>;
  ...
  setHexes(hexesOrCoordinates: Iterable<T | HexCoordinates>): this {
    for (const hexOrCoordinates of hexesOrCoordinates) {
      const hex =
        hexOrCoordinates instanceof Hex ? hexOrCoordinates : new this.#hexClass(hexOrCoordinates);
      this.#setHex(hex);
    }
    return this;
  }
  ...
  #createHexesFromIterableOrTraversers(
    ...
    return (
      this.#isTraverser(input) ? this.#callTraverser(input)
      : Array.isArray(input) && this.#isTraverser(input[0]) ? this.#callTraverser(concat(input))
