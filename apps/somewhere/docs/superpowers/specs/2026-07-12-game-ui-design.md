...
- **`close()` vs `destroy()`:** `close()` is the user-facing path — enters `closing`, runs the optional fade-out, and on completion pops the focus scope, removes and destroys the views, and clears `openModal`. `destroy()` is the teardown path — cancels any in-flight tween, pops the scope if one is still pushed (tolerant of an already-empty stack), and synchronously removes + destroys, callable from any state. Reentrancy is defined by the state machine: `open()` is a no-op unless `closed` (except the reopen-during-`closing` reversal above); `close()` is a no-op while `closing` or `closed`.
...
- **Behavior:** the Escape handler branches on **`world.isPaused`** (the single authoritative source; the pause/resume guards throw on any invariant violation, keeping bugs loud).
  - Open (Escape while unpaused, or the corner button): `world.pause()` then `modal.open()`.
  - **Resume** (Resume button, or Escape while paused — Escape toggles): `modal.close()` then `world.resume()` — the world unfreezes at close-**start**, behind the fading scrim, not after the fade completes.
  - **Escape spam is safe by the state machine:** Escape during the fade-in takes the close path (cancel + replace, state → `closing`); Escape during the fade-out re-pauses and reopens the same instance (cancel + replace, state → `opening`).
...
- **`Modal`:** open adds scrim + content and pushes a focus scope; a completed close pops it and restores prior focus; state-machine reentrancy (`open()` no-op unless `closed`, `close()` no-op while `closing`/`closed`); toggling mid-fade cancels the running tween and replaces it with one starting from the current alpha (close during `opening`; reopen of the same instance during `closing`); `destroy()` cancels any in-flight tween from any state; owning-screen hide/destroy cleans up an open modal.
