...
**Spec:** `docs/superpowers/specs/2026-07-12-game-ui-design.md` (read it for rationale; this plan is self-sufficient for execution).

**Goal:** Restructure the app's single demo screen into a conventional game flow — main menu → game screen with a pause overlay that truly freezes the ECS world — plus an Options modal backed by an in-memory settings module.

**Architecture:** Pausing is one axiom: a guard as the first statement of `World.update()`; everything that animates must therefore be driven from the world's update path (sprites and map tiles switch to `autoUpdate: false` and are advanced by their owning systems). The pause menu is not a screen but a new reusable engine `Modal` primitive (scrim + focus scope + optional fade), which ships together with a lazy self-heal for stale focus scopes in `UiRoot`. The current `mainScreen` is dismantled into `mainMenuScreen` and `gameScreen` in a single task so the app boots at every commit.

**Tech Stack:** TypeScript, Pixi.js v8, @pixi/layout (yoga flexbox on pixi containers), custom ECS (`source/engine/ecs`), Vitest 4 + happy-dom, React Router (mount shell only).

## Global Constraints

Copied from the spec — every task implicitly includes these:

- **All game UI is Pixi-canvas-only.** React is just the mount shell; never add DOM/React UI. (The existing `TextInput` hidden-DOM-input mechanism is the sanctioned exception.)
- **Strings stay hardcoded literals** (no i18n exists). Exact copy: `"Somewhere"`, `"New Game"`, `"Options"`, `"Pause"`, `"Paused"`, `"Resume"`, `"Quit to menu"`, `"Player name"`, `"Sound"`, `"Close"`, `"Name..."`, `"Wall hits: N"`.
- **No Escape-key handling anywhere in v1.** Keyboard users use the focus system (Tab/arrows + Enter/Space).
...
- **Settings are in-memory only** — values reset on reload; a comment marks localStorage as the intended future upgrade.
- **Red-Green TDD** for all engine/logic code: write the failing test, run it, confirm it fails for the expected reason, then implement minimally. Screens and widget-construction glue are exempt (the suite has no real-screen harness — spec §7); they are covered by typecheck, lint, and the manual visual pass.
- **The app must boot at every commit.** The `mainScreen.ts` deletion and the `_index.tsx` import flip happen in the same task (Task 8).
- **Test-file naming:** the screen-logic test file is `tests/pauseFlow.test.ts` — deliberately NOT `tests/gameScreen.test.ts`, which collides with the existing `tests/GameScreen.test.ts` by case alone on case-insensitive filesystems.
...
- Run one test file: `npx vitest run tests/World.test.ts` (vitest is configured in `vite.config.ts`, environment happy-dom). Full suite: `npm test`. Typecheck: `npm run typecheck`. Lint: `npm run lint`.
- Real `pixi.js` imports work headless in tests (`World.test.ts` builds real `pixi.Container`s; `pixi.Texture.WHITE` needs no renderer). Some test files instead `vi.mock('pixi.js', ...)` entirely (`UiRoot.test.ts`) — each file picks one approach.
- Code style: `let` over `const` for locals, private fields as `#name`, teardown via `DisposableStack` + `defer`, blank line after `if` guards, tests use `describe`/`test` (never `it`).
- Commit style (match `git log`): sentence-case imperative subject, no `feat:`/`fix:` prefixes. Example: `Add pause/resume to World`.
...
**Tests:** Modify `tests/World.test.ts`, `tests/UiRoot.test.ts`, `tests/graphicsSystem.test.ts`. Create `tests/Sprite.test.ts`, `tests/Map.test.ts`, `tests/mapSystem.test.ts`, `tests/Modal.test.ts`, `tests/settings.test.ts`, `tests/pauseFlow.test.ts`.

**Task order & dependencies:** 1–7 are independent of each other except 5 (Modal) needs 4 (UiRoot fix ships with the modal). 8 needs 1, 5, 6, 7. 9 needs 8. Execute in numeric order.
...
- Produces: `World.pause(): void` (throws `'World is not running!'` / `'World is already paused!'`), `World.resume(): void` (throws `'World is not paused!'`), `get isPaused(): boolean`. `stop()` works on a paused world and resets the flag. Consumed by Task 7 (structurally) and Task 8.

- [ ] **Step 1: Write the failing tests**

Append this `describe` block inside the top-level `describe('World', ...)` in `tests/World.test.ts` (after the `'World.update deferred structural changes'` block). Everything it uses (`World`, `System`, `Entity`, `EventChannel`, `BarEvent`) is already imported in the file.
...
Expected: FAIL — the new tests error with `world.pause is not a function` / `expected undefined to be false` (`isPaused` doesn't exist yet); all pre-existing tests still pass.
...
Run: `npm run typecheck && npm run lint && npx vitest run tests/World.test.ts`
Expected: all clean.
...
**Interfaces:**
- Consumes: `pixi.AnimatedSprite` two-arg constructor `(frames, autoUpdate)`; `AnimatedSprite.update(ticker)` (advances only while `playing` — verified against pixi 8.15).
- Produces: engine `Sprite` whose `AnimatedSprite`s never advance on Pixi's shared clock; `graphicsSystem` advances the current sprite each world update. No signature changes.

**Testing rationale (spec §7 "world-driven animations"):** the freeze chain is covered piecewise — (a) sprites are off Pixi's clock (`autoUpdate: false`), (b) `graphicsSystem` advances them on the world update path, (c) a paused world runs no systems (Task 1). The `Sprite` tests use real pixi (`Texture.WHITE` works headless); the system test uses the file's existing stub pattern.
...
Expected: FAIL — first test: `expected true to be false` (`autoUpdate` defaults to `true`). (The second test pins behavior that must not regress; it may already pass.)

- [ ] **Step 3: Implement the `Sprite` change**

In `source/engine/graphics/Sprite.ts`, replace the `AnimatedSprite` construction line inside the constructor loop:
...
Expected: FAIL — `expected "spy" to be called 1 times, but got 0 times`. The pre-existing test still passes.

- [ ] **Step 7: Implement the `graphicsSystem` change**

In `source/game/graphicsSystem.ts`, add as the last statement of the `for (let entity of system.entities)` loop in `onUpdate` (after the `zIndex` assignment):
...
Run: `npx vitest run tests/graphicsSystem.test.ts tests/Sprite.test.ts`
Expected: PASS.
...
**Interfaces:**
- Consumes: real `Tilemap`/`Tileset` (plain-data constructors; `Map` checks them with `instanceof`), `toTileGid`/`toTileId` converters.
- Produces: `Map.update(ticker: pixi.Ticker): void` — advances all animated tile sprites; called by `mapSystem` once per entity per frame.
...
Expected: FAIL — `autoUpdate` is `true`, and `map.update is not a function`.
...
Replace the animated branch of the tile construction (`if (tilesetTile.textures.length <= 1) { ... } else { ... }` — only the `else` body changes):
...
Run: `npx vitest run tests/mapSystem.test.ts`
Expected: FAIL — `expected "spy" to be called 1 times, but got 0 times`.

- [ ] **Step 7: Implement the `mapSystem` change**

In `source/game/mapSystem.ts`, replace `onUpdate` (the first parameter is renamed `delta` → `ticker` — it is a `pixi.Ticker`):
...
Run: `npx vitest run tests/mapSystem.test.ts tests/Map.test.ts`
Expected: PASS.
...
**Interfaces:**
- Consumes: nothing new.
- Produces: no API change. Behavior: before every focus walk, `#collectFocusables` prunes from the top of `#scopes` every scope whose root view is `destroyed` or detached from the ui root's view (checked via the pixi `view.parent` chain), restoring the last-pruned scope's `previousFocus` if still collectible — mirroring `popFocusScope`. This is the engine fix that ships with the modal (spec §4); the Modal (Task 5) pops its scope properly in all designed flows — this is the safety net for out-of-band removals.

- [ ] **Step 1: Align the test mock with real pixi**

In `tests/UiRoot.test.ts`, the mock `Container.removeChild` must null the child's `parent` (real pixi does; the self-heal's attachment check depends on it — verified against pixi 8.15). Replace the mock's `removeChild`:
...
Run: `npx vitest run tests/UiRoot.test.ts` — Expected: PASS (pre-existing tests are unaffected by the mock fix).

- [ ] **Step 2: Write the failing tests**

Append this `describe` block inside the top-level `describe('UiRoot', ...)` (after the `'focus scopes'` block):
...
```ts
  describe('focus scope self-heal (out-of-band removal)', () => {
    ...
    test('deep removal (subtree detached below the ui root) is healed the same way', () => {
      ...
      (outer.view as unknown as MockContainer).removeChild(
        inner.view as unknown as MockContainer,
      );
...
Expected: FAIL — in the first test `root.focused` is `b` (the stale scope keeps the detached subtree focusable); the other new tests fail similarly. All pre-existing tests pass.

- [ ] **Step 4: Implement the self-heal**

In `source/engine/ui/UiRoot.ts`:

Replace the long `// TODO: Investigate further how focus scopes should interact with removal. ...` comment block above `pushFocusScope` (lines starting `// TODO: Investigate` through `// should be removed.`) with:
...
**Interfaces:**
- Consumes: `UiRoot.addChild/removeChild/pushFocusScope/popFocusScope/focus` (existing), `Scheduler.tween` cancel handle (existing), Task 4's self-heal (as documented safety net only).
- Produces (used by Tasks 8–9):
  ...
  - `close(): boolean` — returns whether THIS call initiated the close; `false` while `closing`/`closed`. (Plan-level addition to the spec: Resume stays activatable during the fade-out — scope pops at close-complete — so the caller needs this to avoid a second `world.resume()`, which throws.)
  ...
  - `onClose` fires when a `close()` completes (plan-level addition: it is how the owning screen clears its `openModal` reference, which the spec requires to stay set "until a close completes or destroy() runs").
...
Run: `npx vitest run tests/Modal.test.ts`
Expected: FAIL — `Failed to load ../source/engine/ui/Modal.js` (module does not exist).
...
Run: `npx vitest run tests/Modal.test.ts`
Expected: FAIL — TypeScript/runtime: `scheduler`/`fadeDuration` are not valid options yet; state is `'open'` immediately instead of `'opening'`.
...
Run: `npx vitest run tests/Modal.test.ts`
Expected: PASS (all lifecycle + fade tests).
...
**Interfaces:**
- Produces: `settings: {playerName: string; soundEnabled: boolean}` — a plain mutable object. Consumed by Task 8 (HUD reads `playerName` in `onShow`) and Task 9 (Options writes both). `soundEnabled` has no consumer yet (no sound system exists — deliberate no-op).
...
Expected: FAIL — `Failed to load ../source/game/settings.js`.
...
**Interfaces:**
- Consumes: structural contracts only (`{pause}`, `{resume}`, `{stop}`, `{close(): boolean}`, `{destroy()}`) so tests pass plain fakes — matched at the call site by `World` (Task 1) and `Modal` (Task 5).
- Produces (consumed by Task 8's `gameScreen`):
  - `openPauseMenu({world, openModal}): void` — pause first, then open
  - `resumeFromPause({world, modal}): void` — close at close-start, resume only if this call initiated the close
  - `teardownGameScreen({modal, world, detachWorld}): void` — modal destroy → world stop → detach, in that order
...
Run: `npx vitest run tests/pauseFlow.test.ts`
Expected: FAIL — `Failed to load ../source/game/pauseFlow.js`.
...
This is the one task without unit tests (spec §7: screens are not unit-tested; their extracted logic was tested in Task 7). It is verified by typecheck, lint, the full suite, and a manual smoke pass. Everything lands in ONE commit so the app boots at every commit.
...
**Interfaces:**
- Consumes: `World.pause/resume/stop` (Task 1), `Modal` (Task 5), `settings.playerName` (Task 6), `openPauseMenu/resumeFromPause/teardownGameScreen` (Task 7), existing `game`, `world`, `uiEvents`, `loadingScreen`, `Button`, `Panel`, `Text`, `Container`, `GameScreen`.
- Produces: `mainMenuScreen: GameScreen<{openModal: Modal | null}>`, `gameScreen: GameScreen<{hitCounter: Text; nameLabel: Text; openModal: Modal | null; pauseButton: Button}, UIEventMap>`, and from `widgets.ts`: `nineSlice(name, slice)`, `createButton({label, onClick, fontSize?, layout?})`, `BUTTON_SLICE`, `BUTTON_ACTIVE_SLICE`, `BANNER_SLICE`, `INPUT_SLICE`, `FOCUS_RING`. Task 9 consumes `mainMenuScreen`'s `MainMenuScreenState` and `widgets.ts`.

**Spec §6 mapping (fate of every `mainScreen` piece):** title → menu; wall-hit counter → HUD (still the live `uiBridge`/`wallHitChannel` consumer); name `TextInput` → Options (Task 9); "Sound" toggle → Options (Task 9); "Enable sound" meta-toggle → deleted (disabled-state coverage lives in widget unit tests); reminder-dialog demo → deleted (Modal supersedes it); `mainScreen.ts` → deleted here.

- [ ] **Step 1: Create `source/game/widgets.ts`**

Shared widget construction for the three UI surfaces (menu, pause modal, Options modal) — pure DRY extraction of what `mainScreen.ts` built inline:
...
(`openModal` stays `null` until Task 9 adds the Options modal; the lifecycle plumbing is identical to `gameScreen`'s, so it lands here.)
...
Run: `npm run typecheck && npm run lint && npm test`
Expected: all clean; the full suite passes (no test imported `mainScreen`).

- [ ] **Step 7: Manual smoke pass**

Run: `npm run develop` and open `http://localhost:5000`. Walk:

1. Boots **directly into the menu** (no loading screen at boot): black background, centered banner, "Somewhere", New Game.
2. **New Game** → loading screen appears iff the `game` bundle is still cold → world with tilemap + character; HUD top-left (empty name line + `Wall hits: 0`), Pause button top-right.
3. Click to move; walk into a wall → counter increments (the ECS→UI bridge still has its live consumer).
4. **Pause** → world freezes mid-step (character animation AND animated map tiles hold), dark scrim, centered "Paused" panel fades in. Clicking the world does nothing (scrim blocks click-to-move).
5. Keyboard: arrows/Tab cycle only Resume/Quit (focus ring visible above the scrim); Enter activates. Resume holds initial focus (no ring until the first key).
6. **Resume** → panel fades out, world unfreezes immediately (at fade start), animations continue from the held frame.
7. Pause → **Quit to menu** → back at the menu instantly. **New Game** again → fresh run (counter 0, character at start).
8. Resize the window at the menu, in game, and while paused → banner stays centered, HUD corners stick, scrim keeps covering.
...
**Interfaces:**
- Consumes: `Modal` (Task 5), `settings` (Task 6), `TextInput`/`Toggle`/`Container` (existing), `INPUT_SLICE`/`createButton`/`nineSlice`/`BANNER_SLICE` (Task 8).
...
1. Menu shows New Game, Options. **Options** → modal fades in over the menu (menu buttons unreachable under the scrim); nothing is focused until the first Tab/arrow.
2. Type a name into Player name; flip Sound; **Close** → modal fades out; press Tab — the ring reappears on the Options button (focus restored via the scope pop).
3. Reopen Options → the name field and toggle show the values just set (constructed per open from `settings`).
4. **New Game** → the HUD's top-left shows the entered name above the wall-hit counter.
5. Quit to menu → Options still shows the name (in-memory persistence within the session). Reload the page → values reset (expected: in-memory only).
...
Expected: full suite green (including all new test files: `Sprite`, `Map`, `mapSystem`, `Modal`, `settings`, `pauseFlow`, plus the extended `World`, `UiRoot`, `graphicsSystem`), typecheck and lint clean.

- [ ] **Step 2: Full manual visual pass (spec §7)**

Run: `npm run develop` and walk the complete loop in one session: menu → Options (set name, toggle sound, close, focus returns) → New Game (loading screen on cold bundle; HUD shows name) → move / hit walls → Pause (freeze, scrim, keyboard trap, ring above scrim) → Resume (fade, world resumes at fade start) → Pause → Quit to menu → New Game (fresh run) — plus window resizes at each station.

- [ ] **Step 3: Done**

If anything failed, fix it in the offending task's files (keeping its tests green) and commit as a follow-up (e.g. `Fix <thing> in <area>`). Do not merge/PR yet — branch finishing is a separate decision (use superpowers:finishing-a-development-branch).

---

## Plan-level decisions (deviations & additions to the spec, with rationale)

1. **`Modal.close(): boolean`** (spec has `close(): void`): the focus scope pops at close-complete, so Resume stays activatable during the fade-out; the boolean ("did this call initiate the close?") lets `resumeFromPause` avoid a second `world.resume()`, which throws by design. Without it the spec's own guards make double-Resume crash.
2. **`Modal` `onClose` option** (not in the spec's options list): the spec requires the owning screen's `openModal` to stay set "until a close completes"; only the modal knows when the fade-out completes, so it needs a completion hook.
3. **`Modal` root layout gets `position: 'absolute', left: 0, top: 0`**: both screens give their `ui.view` a flex layout (menu centering, HUD row); without absolute positioning the full-screen modal would participate in that flex flow. `@pixi/layout` supports it (yoga). The caller's `layout` is still spread after, so content placement remains verbatim.
4. **`source/game/widgets.ts`**: `nineSlice`, slice constants, `createButton`, `FOCUS_RING` — pure DRY extraction of construction code `mainScreen.ts` had inline, now needed by three surfaces. No unit tests (construction glue; covered by typecheck + manual pass).
5. **Animation tests use real pixi headlessly** (`Texture.WHITE`, real `Tilemap`/`Tileset` instances, `Assets.get` spies): verified to work in this environment; it tests real `AnimatedSprite.update` semantics instead of a mock of them. `AnimatedSprite.update()` no-ops while not playing (verified), so unconditional calls in the systems are safe.
6. **`mapSystem`'s `onUpdate` param renamed `delta` → `ticker`**: it is a `pixi.Ticker` and is now passed through to `map.update(ticker)`; the old name was misleading at the new call site.
