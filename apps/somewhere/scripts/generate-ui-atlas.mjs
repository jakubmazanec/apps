...
// Slider track: a fixed-size horizontal groove — a 1-px border ring around a
// flat body, same shape as buildBox at an arbitrary width/height. Unlike the
// fill, Slider never resizes this box after construction (its width comes
// straight from this frame's natural size, the same fixed-size approach
// buildToggle's frames use).
const SLIDER_TRACK_W = 32;
const SLIDER_TRACK_H = 6;

function buildSliderTrack({border, body}) {
  let cells = [];

  for (let row = 0; row < SLIDER_TRACK_H; row++) {
    let cols = [];

    for (let col = 0; col < SLIDER_TRACK_W; col++) {
      let isBorder =
        row === 0 || row === SLIDER_TRACK_H - 1 || col === 0 || col === SLIDER_TRACK_W - 1;
...
// Slider fill: a small solid-color block. Slider resizes it at runtime via
// Container.setSize (the same generic call swapBackground.ts uses for every
// other widget background), so its authored size only needs to be small — a
// solid color has no corner detail to distort under that stretch.
const SLIDER_FILL_SIZE = 4;

function buildSliderFill() {
  let cells = [];

  for (let row = 0; row < SLIDER_FILL_SIZE; row++) {
    let cols = [];

    for (let col = 0; col < SLIDER_FILL_SIZE; col++) {
...
let frames = [
  ...
  {
    name: 'slider-track',
    image: renderCells(buildSliderTrack({border: palette.border, body: palette.navy})),
  },
  {
    name: 'slider-track-hovered',
    image: renderCells(buildSliderTrack({border: palette.borderBright, body: palette.navyBright})),
  },
  {
    name: 'slider-track-disabled',
    image: renderCells(buildSliderTrack({border: palette.borderMuted, body: palette.navyMuted})),
  },
  {name: 'slider-fill', image: renderCells(buildSliderFill())},
