...
export class Game {
  ...
  on<T extends EventEmitter.EventNames<pixi.DisplayObjectEvents>>(
    event: T,
    fn: EventEmitter.EventListener<pixi.DisplayObjectEvents, T>,
  ): this {
  ...
  once<T extends EventEmitter.EventNames<pixi.DisplayObjectEvents>>(
    event: T,
    fn: EventEmitter.EventListener<pixi.DisplayObjectEvents, T>,
  ): this {
  ...
  off<T extends EventEmitter.EventNames<pixi.DisplayObjectEvents>>(
    event: T,
    fn?: EventEmitter.EventListener<pixi.DisplayObjectEvents, T>,
  ): this {
    ...
    await pixi.Assets.init({
      manifest: {
        bundles: assetBundles.map(({name, assets}) => ({
          name,
          assets: assets.map(({name, sources}) => ({alias: name, src: sources})),
        })),
      },
    });
  ...
  addRef(ref: React.RefObject<Element>) {
    ref.current?.appendChild(this.app.view as unknown as Element);
  ...
  removeRef() {
    this.ref?.current?.removeChild(this.app.view as unknown as Element);
  ...
  resize = () => {
    if (this.ref?.current) {
      ...
      // this.pixi.app.renderer.view.style ??= {};
