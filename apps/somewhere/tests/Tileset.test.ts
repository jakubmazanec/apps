...
function stubImage() {
  vitest.spyOn(pixi.Assets, 'load').mockResolvedValue(undefined as never);
  vitest
    .spyOn(pixi.Texture, 'from')
    .mockReturnValue(new pixi.Texture({source: new pixi.TextureSource({width: 16, height: 32})}));
}
