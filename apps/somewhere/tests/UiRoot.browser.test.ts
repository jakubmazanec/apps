...
describe(UiRoot, () => {
  ...
  describe('pointer interplay', () => {
    test('a tap inside a focusable component silently focuses it', () => {
      ...
      // addEventListener with {capture: true} registers under `<type>capture`
      // on the real pixi emitter (EventBoundary dispatches that key during the
      // capture phase).
      view.emit('pointertapcapture', {target: inner});
    ...
    test('a tap outside any focusable changes nothing', () => {
      ...
      view.emit('pointertapcapture', {target: plain});
    ...
    test('a tap on a non-focusable component changes nothing', () => {
      ...
      view.emit('pointertapcapture', {target: component.view});
    ...
    test('a tap that bubbles to the UI root is stopped before reaching the game view', () => {
      let root = createRoot();
      let view = root.view as unknown as MockContainer;
      let event = {target: null, stopPropagation: vitest.fn<() => void>()};
  ...
  describe('increase / decrease', () => {
    test('increases the focused component', () => {
      let a = focusable();
      let root = createRootWith(a);
      ...
      expect(a.increase).toHaveBeenCalledTimes(1);
    });

    test('decreases the focused component', () => {
      let a = focusable();
      let root = createRootWith(a);
      ...
      expect(a.decrease).toHaveBeenCalledTimes(1);
    });

    test('both are no-ops with nothing focused', () => {
      let a = focusable();
      let root = createRootWith(a);
      ...
      expect(a.increase).not.toHaveBeenCalled();
      expect(a.decrease).not.toHaveBeenCalled();
    });

    test('drop focus instead of calling increase on a stale component', () => {
      let a = focusable();
      let root = createRootWith(a);
  ...
  describe('focus events', () => {
    test('fires a move event when focus lands on a new component', () => {
      let onFocusEvent = vitest.fn<(event: UiFocusEvent) => void>();
