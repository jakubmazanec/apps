...
describe('motionSystem frame rate independence', () => {
  test('one big frame covers the same distance as many small frames', () => {
    let {world, motion} = createWorldWithPlayerNearWall();

    world.start();

    // Open space: player at x 20 with the wall at x 32, an 8px box, moving at
    // MAX_SPEED (1). One frame of deltaTime 4 must advance the full 4 art-px
    // (the distance four deltaTime-1 frames would cover), not a capped 2.
    motion.target = undefined;
    motion.velocity.x = 1;
    motion.velocity.y = 0;
    world.update(tick(4));

    expect(motion.position.x).toBe(24);

    world.stop();
  });
});

describe('motionSystem tap-to-move arrival', () => {
  test('lands on the target at deltaTime 2 instead of oscillating past it', () => {
    let {world, motion} = createWorldWithPlayerNearWall();

    world.start();

    // Target 0.5 art-px away, well inside open space (wall is at x 32). At
    // deltaTime 2 the uncapped step would be velocity (0.5) * 2 = 1.0, landing
    // at 21 and then bouncing back to 20 forever. The arrive-clamp caps the
    // step to the remaining 0.5 so the player lands exactly on the target.
    motion.position.x = 20;
    motion.position.y = 0;
    motion.target = new Vector(20.5, 0);
    world.update(tick(2));

    expect(motion.position.x).toBe(20.5);

    // On the next frame velocity zeroes out and the existing zero-delta check
    // clears the target.
    world.update(tick(2));
