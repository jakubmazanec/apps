...
import {DialogueCommand, type DialogueCommandType} from '../source/game/events/DialogueCommand.js';
...
import {dialogueSystem} from '../source/game/systems/dialogueSystem.js';
...
describe('dialogueSystem', () => {
  ...
  test('interact while choosing confirms the highlighted choice', () => {
    // E is the whole conversation on one key: it starts, pages, and confirms,
    // so the hand never moves to Enter. Enter and Space still confirm too,
    // through the choice buttons' focus activate.
    let {world, component} = createHarness([createNpc()]);
    ...
    expect(component.active?.pageText).toBe('This way.'); // choice 0 followed
  });
