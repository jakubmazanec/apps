...
describe(Vector, () => {
  describe('construction and basic state', () => {
    ...
    test('isZero is false when a component is exactly Number.EPSILON', () => {
      expect(new Vector(Number.EPSILON, 0).isZero).toBe(false);
      expect(new Vector(0, Number.EPSILON).isZero).toBe(false);
      expect(new Vector(-Number.EPSILON, 0).isZero).toBe(false);
    });
  ...
  describe('equality', () => {
    ...
    test('isEqual defaults to comparing against ORIGIN', () => {
      expect(new Vector(0, 0).isEqual()).toBe(true);
      expect(new Vector(1, 0).isEqual()).toBe(false);
    });

    test('Vector.ORIGIN is frozen', () => {
      expect(Object.isFrozen(Vector.ORIGIN)).toBe(true);
      expect(() => {
        Vector.ORIGIN.x = 5;
      }).toThrow(TypeError);
    });
