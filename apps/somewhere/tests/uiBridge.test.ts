...
function withBridge(run: (world: World) => void) {
  ...
  // update() is a no-op on a world that is not running, so the bridge only sees a frame
  // once the world has started.
  world.start();
...
describe('uiBridge', () => {
  ...
  test('emits one event per buffered event, in push order', () => {
    ...
    withBridge((world) => {
      ...
      // eslint-disable-next-line unicorn/prefer-single-call -- EventChannel#push takes one event at a time, unlike Array#push
