export type ChangeShipEffectConfig =
  ...
  | {
      duration: PermanentEffectDuration;
      type: 'change-ship';
      propertyName: 'crew' | 'hull' | 'sails';
      bonus: number;
    };
...
export type EffectConfig = ChangeShipEffectConfig;
...
export class Effect {
  ...
  //     case 'change-ship': {
  //       if (this.config.propertyName === 'ammo') {
  //         effectDescription = `${this.config.propertyName.charAt(0).toUpperCase() + this.config.propertyName.slice(1)} (ship): =${this.config.value}`;
  //       } else {
